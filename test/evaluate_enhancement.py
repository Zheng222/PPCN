...
if __name__ == "__main__":
    ...
    if compute_running_time:
        ...
        avg_time_baseline, max_ram = utils.compute_running_time("superres", "models_pretrained/dped_srcnn.pb",
                                                                "dped/HD_res/")
        ...
        scoreA = 4 * (PSNR_solution - 21) + 100 * (SSIM_solution - 0.9) + 2 * min(
            float(avg_time_baseline) / avg_time_solution, 4.0)
        scoreB = 1 * (PSNR_solution - 21) + 400 * (SSIM_solution - 0.9) + 2 * min(
            float(avg_time_baseline) / avg_time_solution, 4.0)
        scoreC = 2 * (PSNR_solution - 21) + 200 * (SSIM_solution - 0.9) + 2.9 * min(
            float(avg_time_baseline) / avg_time_solution, 4.0)

        print("Score A: %.4f, Score B: %.4f, Score C: %.4f" % (scoreA, scoreB, scoreC))
