...
def normalized(x):
    # step 1 vectorized
    _, h, w, _ = x.get_shape().as_list()  # c=1
    x = tf.reshape(x, (-1, h*w))
    x = tf.nn.l2_normalize(x, axis=-1)
    return x

def kd_loss(x, y): # knowledge distillation, x and y are list
    x = [normalized(i) for i in x]
    y = [normalized(j) for j in y]
    loss = [tf.nn.l2_loss(m - n) for m, n in zip(x, y)]
    loss = tf.add_n(loss)
    return loss

def ssim(x, y, max_val=1.0):
    return tf.reduce_sum(tf.image.ssim(x, y, max_val))

def log10(x):
    numerator = tf.log(x)  # log_e
    denominator = tf.log(tf.constant(10, dtype=numerator.dtype))
    return numerator / denominator

def _tensor_size(tensor):
    from operator import mul
    return reduce(mul, (d.value for d in tensor.get_shape()[1:]), 1)  # [H, W, C], 1 can be ignored

def gauss_kernel(kernlen=21, nsig=3, channels=1):
    interval = (2*nsig+1.)/(kernlen)
    x = np.linspace(-nsig-interval/2., nsig+interval/2., kernlen+1)
    kern1d = np.diff(st.norm.cdf(x))  # cdf
...
def blur(x):  # employ guassian blur operation
...
def process_command_args(arguments):
    ...
    if phone == "":
        ...
        print("python train_teacher.py model={iphone,blackberry,sony}\n")
        sys.exit()
