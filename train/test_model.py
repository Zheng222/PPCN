...
with tf.Session(config=config) as sess:
    ...
    if phone.endswith("_orig"):
        ...
        num_saved_models = int(len([f for f in os.listdir("models_teacher/") if f.startswith(phone + "_iteration")]) / 2)
        ...
        for i in iteration:
            ...
            # load pre-trained model
            saver = tf.train.Saver()
            saver.restore(sess, "models_teacher/" + phone + "_iteration_" + str(i) + ".ckpt")
