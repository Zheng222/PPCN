...
class CSFlow:
    ...
    def reversed_direction_CS(self):
        ...
        work_axis = [TensorAxis.H, TensorAxis.W]  # [1, 2]
    ...
    def create_using_L2(I_features, T_features, sigma = float(0.1), b = float(1.0)):
        ...
        with tf.name_scope('CS'):
            ...
            sT = T_features.shape.as_list()  # shape target
            sI = I_features.shape.as_list()  # shape input

            Ivecs = tf.reshape(I_features, (sI[TensorAxis.N], -1, sI[TensorAxis.C]))  # input vector  [N, H*W, C]
            Tvecs = tf.reshape(T_features, (sI[TensorAxis.N], -1, sT[TensorAxis.C]))  # target vector [N, H*W, C]
            r_Ts = tf.reduce_sum(Tvecs * Tvecs, 2)  # square and sum
            r_Is = tf.reduce_sum(Ivecs * Ivecs, 2)
            raw_distances_list = []
            for i in range(sT[TensorAxis.N]):  # batch
                Ivec, Tvec, r_T, r_I = Ivecs[i], Tvecs[i], r_Ts[i], r_Is[i]
                A = Tvec @ tf.transpose(Ivec)  # matmul
                cs_flow.A = A
                # A = tf.matmul(Tvec, tf.transpose(Ivec))
                r_T = tf.reshape(r_T, [-1, 1])  # turn to column vector  [H*W*C, 1]
                dist = r_T - 2 * A + r_I
    ...
    #--
    @staticmethod
    def create_using_dotP(I_features, T_features, sigma = float(1.0), b = float(1.0), batch_size=1):
        cs_flow = CSFlow(sigma, b)
        with tf.name_scope('CS'):
            # prepare feature before calculating cosine distance
            T_features, I_features = cs_flow.center_by_T(T_features, I_features)  # sub mean
            ...
            with tf.name_scope('IFeatures'):
                ...
                cosine_dist_l = []
                N, _, __, ___ = T_features.shape.as_list()  # [N,H,W,C]
                if N is None:
                    N = batch_size
                for i in range(N):
                    T_features_i = tf.expand_dims(T_features[i, :, :, :], 0)  # [1, H, W, C]
    ...
    # --
    @staticmethod
    def create(I_features, T_features, distance : enums.Distance, nnsigma=float(1.0), b=float(1.0), batch_size=1):
    ...
    def patch_decomposition(self, T_features):  #  T_features [1, H, W, C]
        # patch decomposition
        ...
        patches_as_depth_vectors = tf.extract_image_patches(
            ...
            name='patches_as_depth_vectors')  # extract patches from images and put them in the "depth" output dimension. [1, H, W, 1*1*C]

        self.patches_NHWC = tf.reshape(
            patches_as_depth_vectors,
            shape=[-1, patch_size, patch_size, patches_as_depth_vectors.shape[3].value],  # [H*W, 1, 1, C]
            name='patches_PHWC')
