...
with tf.Graph().as_default(), tf.Session() as sess:
    ...
    loss_discrim = -tf.reduce_sum(discrim_target * tf.log(tf.clip_by_value(discrim_predictions, 1e-10, 1.0)))  # Note: here use tf.reduce_sum, not use tf.reduce_mean
    ...
    generator_vars = [v for v in tf.global_variables() if v.name.startswith("generator_t")]  # update teacher network
    ...
    # load teacher net from checkpoint if exist
    ckpt = tf.train.get_checkpoint_state('models_teacher/')
    ...
    logs = open('models_teacher/' + phone + '.txt', "w+")
    logs.close()
    ...
    for i in range(num_train_iters):
        ...
        if i % eval_step == 0:
            ...
            logs = open('models_teacher/' + phone + '.txt', "a")
            ...
            saver.save(sess, 'models_teacher/' + str(phone) + '_iteration_' + str(i) + '.ckpt', write_meta_graph=False)
