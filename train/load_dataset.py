...
def load_test_data(phone, dped_dir, IMAGE_SIZE):
    ...
    NUM_TEST_IMAGES = len([name for name in os.listdir(test_directory_phone)
                           if os.path.isfile(os.path.join(test_directory_phone, name))])  # num test images

    test_data = np.zeros((NUM_TEST_IMAGES, IMAGE_SIZE))  # image_size = height * width * channel(3)
    test_answ = np.zeros((NUM_TEST_IMAGES, IMAGE_SIZE))  # test_label

    for i in range(0, NUM_TEST_IMAGES):
        
        I = np.asarray(misc.imread(test_directory_phone + str(i) + '.jpg'))
        I = np.float16(np.reshape(I, [1, IMAGE_SIZE]))/255  # [0, 255] --> [0, 1]
        test_data[i, :] = I
...
def load_batch(phone, dped_dir, TRAIN_SIZE, IMAGE_SIZE):
    ...
    i = 0
    for img in TRAIN_IMAGES:  # size(TRAIN_IMAGES) = size(TRAIN_SIZE)

        imgIn = np.asarray(misc.imread(train_directory_phone + str(img) + '.jpg'))
        imgTar = np.asarray(misc.imread(train_directory_dslr + str(img) + '.jpg'))

        imgIn, imgTar = augment(imgIn, imgTar)  # augment

        imgIn = np.float16(np.reshape(imgIn, [1, IMAGE_SIZE])) / 255
        imgTar = np.float16(np.reshape(imgTar, [1, IMAGE_SIZE])) / 255
...
def augment(imgIn, imgTar, flip=True, rot=True):
